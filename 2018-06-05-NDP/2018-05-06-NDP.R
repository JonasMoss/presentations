...
dat_generator <- function(truth) {
  ...
  data.frame(y = y, y1 = y1, y2 = y2, y3 = y3)
  
}
truth = data.frame(pi = c(0.1, 0.5, 0.4), 
                   mu = c(-3, 0, 3), 
                   sigma = sqrt(c(0.5, 0.75, 1)))
...
fit <- stan("2018-06-05-NDP/2018-05-06-NDP.stan", data = input_dat, iter = 1000, chains = 1)
