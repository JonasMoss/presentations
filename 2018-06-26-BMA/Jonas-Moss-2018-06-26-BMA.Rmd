...
motyl_data = data.frame(z = abs(motyl_between$z),
                  M = motyl_between$M,
                  lower = qt(0.95, df = motyl_between$M),
                  upper = qt(0.025, df = motyl_between$M),
                  dist_indices = rep(1, length(motyl_between$M)))
motyl_data$lower[abs(motyl_between$z) > qt(0.975, df = motyl_between$M)] =
  qt(0.95, df = motyl_between$M)
motyl_data$dist_indices[abs(motyl_between$z) < motyl_data$lower] = 9
motyl_data$dist_indices[abs(motyl_between$z) > qt(0.95, df = motyl_between$M)]
...
# 
# mean_npb = rstan::extract(motylmod_npb_gamma$stan_object)$beta_positive[ , 1]
# sd_npb = rstan::extract(motylmod_npb_gamma$stan_object)$beta_positive[ , 2]
# shape_npb = mean_npb^2/sd_npb^2
# rate_npb = mean_npb/sd_npb^2
# 
# mean_pb = rstan::extract(motylmod_gamma$stan_object)$beta_positive[ , 1]
# sd_pb = rstan::extract(motylmod_gamma$stan_object)$beta_positive[ , 2]
# shape_pb = mean_pb^2/sd_pb^2
# rate_pb = mean_pb/sd_pb^2
# 
# x = seq(0, 3, by = 0.005)
# y_npb = sapply(x, function(x) mean(dgamma(x, shape_npb, rate_npb)))
# y_pb = sapply(x, function(x) mean(dgamma(x, shape_pb, rate_pb)))
...
- How to account for selection for significance?
    ...
    - Goes back to Hedges (1984), recently discussed by Gelman.
...
legend("topright", col = c("red", "black", ""), lwd = c(2, 2, NA), bty = "n",
       legend = c("Corrected (mean = 0.12)", 
                  "Not corrected (mean = 0.58)",
                  "Propensity to p-hack: 0.87 (0.02)"), cex = 1.7)
...
## Questions for Bangert-Drowns, Hurley & Wilkinson
- **Question I**: Are the **\textcolor{red}{red}** effects different from the **\textcolor{black}{black}** effects?
    - Results from conventional random effects meta-analysis:
        - Grand mean: 0.22 (p < 0.0001)
        - \textcolor{red}{red mean} mean: 0.35 (p = 0.01)
        - \textcolor{black}{black mean} mean: 0.12 (p = 0.03)
    - But has the covariate itself been selected for significance?
    
- **Question II**: What is the effect size distribution?
    - Fixed effects or random effects?
    - Does *p*-hacking change the estimated effect sizes?
